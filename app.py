...
class showSong(tk.Frame):
    def __init__(self, parent, controller):
        ...
        buttonSubmit = ttk.Button(self, text = "Submit",
                    command = lambda: songSubmit(value0.get(), value1.get(), value2.get(), value3.get(),value4.get()))
        buttonSubmit.grid(row = 8, column = 2, ipady=5, ipadx=15, padx = 10, pady = 10)

        buttonExit = tk.Button(self, text="Exit",command = exit).grid(row = 8, column = 3, ipady=5, ipadx=15, padx = 10, pady = 10)


def songSubmit(songID, writerID, songName, genre, composition_date):
